    ...
    <div class="container">
	   <div class="row">		
			<form role="search" method="get" id="searchform" action="<?php echo home_url('/'); ?>">
				<div class="col-sm">
				    <input type="text" value="" name="s" id="s" placeholder="Search by Title" />
				</div>
                <div class="col-sm">
                    <h6>Check Categories</h6>
            <?php
                    // Use the wp_dropdown_categories function to generate the dropdown
                    /*$args = array(
                        'show_option_none'   => 'Select Category', // Text to display for the default option
                        'option_none_value'  => '-1', // Value for the default option
                        ...
                        'child_of'           => 0, // Display only the top-level categories
                        'exclude'            => '', // Exclude specific categories by ID
                        'echo'               => 1, // Output the dropdown immediately
                        'selected'           => 0, // ID of the category to be pre-selected
                        'hierarchical'       => 1, // Display categories hierarchically
                        'name'               => 'cat-select', // HTML name attribute value
                        'id'                 => 'category-dropdown', // HTML id attribute value
                        'class'              => 'postform', // CSS class for the dropdown
                        'depth'              => 0, // Depth of the dropdown
                        'tab_index'          => 0, // Tab index for the dropdown
                        'taxonomy'           => 'events_tax', // Taxonomy name (default is 'category')
                        'hide_if_empty'      => false, // Do not hide the dropdown if no categories are found
                    );

                    wp_dropdown_categories( $args );*/

                    $terms = get_terms('events_tax');
                    foreach($terms as $term){ ?>
                        <label><?php echo $term->name; ?></label>
                        <input type="checkbox" name="term_data[]" value="<?php echo $term->term_id; ?>">
                    <?php }
            ?>
